...
{
    public class SetIOCBoundPlatesToProcessing
    {
        public async Task RunAsync(DataServicesClient client, WorkflowContext context, CancellationToken cancellationToken)
        {
            ...
            Console.WriteLine($"  source  plate  {TransportedSources} " + Environment.NewLine);
            Console.WriteLine($"  Destinations  plate  {TransportedDestination}  " + Environment.NewLine);
            ...
            .Where(x => x.Status.ToString() == "Completed")
            .ToList();
            ...
            foreach (var destination in TransportingDestinations)
            {
                ...
                {
                    destination.Properties.SetValue("Status", "Processing");
                    _identityHelper.Register(destination, transportingDestinationJob, RequestedOrder);

                    Console.WriteLine($"  destination  plate  {transportingDestinationName} with ID {transportingDestinationId} and operation {transportingDestinationOperation} was set to PROCESSING " + Environment.NewLine);
